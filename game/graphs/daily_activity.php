<?php
	...
	$days = array();
	$sql = $db->Query("SELECT id,timestamp FROM `[log]` WHERE `playerid`='".$player['id']."' AND `timestamp`<='".$time."'");
	while ($event = $db->FetchArray($sql))
	{
		$days[date('d.m.Y H', $event['timestamp'])]++;
	}
	...
	foreach ($dates as $date)
	{
		...
		$activity = $days[date('d.m.Y H', $date)] ? $days[date('d.m.Y H', $date)] : 0;
		
		$d = new hollow_dot($activity);
		$data['activity'][] = $d->colour('#999999')->size(3)->tooltip('#x_label#<br>#val#');
	...
	$area->set_values($data['activity']);
	$area->set_key('Activitate', 10);
