...
$total_pages = $db->QueryGetNumRows("SELECT id FROM `[affiliates]`".$db_value."");
include('lib/paginare.php');

$affiliates = $db->QueryFetchArrayAll("SELECT id,username,email,balance,reg_time,last_log,IP_regged_with FROM `[affiliates]`".$db_value." ORDER BY reg_time DESC LIMIT ".$start.",".$limita."");
                            ...
                            <div class="grid_12 boxed-table">
                                <div class="box">
                                    <header>
                                        <div class="inner">
                                            <div class="left title">
                                                <h1>Affiliate Accounts (<?=$total_pages?>)</h1>
                                            </div>
                                        </div>
                                    </header>
...
?>
                                                <tr>
                                                    ...
                                                    <td class="center">
                                                        <a href="index.php?page=affiliates&edit=<?=$aff['id']?>" class="tooltip glyph settings" title="Edit"></a>
                                                    </td>
                                                </tr>
<?php
	endforeach;
?>
                                ...
                                <div class="extension bottom inright pagination">
                                    <nav>
                                        <ul>
                                            <li<?=(($pagina <= 1 || $pagina == '') ? ' class="disabled"' : '')?>><?=(($pagina <= 1 || $pagina == '') ? '<span>&laquo; Back</span>' : '<a href="index.php?page=affiliates&p='.($pagina-1).'">&laquo; Back</a>')?></li>
                                            <?=$pagination?>
                                            <li<?=(($pagina >= $lastpage || $pagina == '') ? ' class="disabled"' : '')?>><?=(($pagina >= $lastpage || $pagina == '') ? '<span>Next &raquo; </span>' : '<a href="index.php?page=affiliates&p='.($pagina+1).'">Next &raquo;</a>')?></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
