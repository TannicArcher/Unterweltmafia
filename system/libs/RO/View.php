<?php
	...
	class View
	{
		...
		{
			$months[false] = array("Jan", "Feb", "Mär", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez");
			$months[true]  = array("Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember");
		...
		public static function strTime($s, $long = 0, $value_seperator = ' ', $bold = 1, $emptyMsg = 'Jetzt')
		{
			$y = intval($s/31536000);
			$s -= $y*31536000;
			...
			$text_opts = array(
				'y' => array( array('a', 'a'), array('Jahr', 'Jahre') ),
				'w' => array( array('s', 's'), array('Woche', 'Wochen') ),
				'd' => array( array('z', 'z'), array('Tag', 'Tage') ),
				'h' => array( array('h', 'h'), array('Stunde', 'Stunden') ),
				'm' => array( array('m', 'm'), array('Minute', 'Minuten') ),
				's' => array( array('s', 's'), array('Sekunde', 'Sekunden') ),
			);
		...
		public static function HowLongAgo($time)
		{
			$delta = time() - $time;
			if ($delta < 60) {
				return $delta . ' Sekunden vergangen';
			} else if ($delta < 120) {
				return '~1 Minute vergangen';
			} else if ($delta < (45 * 60)) {
				return floor($delta / 60) . ' Minuten vergangen';
			} else if ($delta < (90 * 60)) {
				return '~1 Stunde vergangen';
			} else if ($delta < (24 * 60 * 60)) {
				return '~' . floor($delta / 3600) . ' Stunden vergangen';
			} else if ($delta < (48 * 60 * 60)) {
				return 'Keine Information!';
			} else {
				return floor($delta / 86400) . ' Tage vergangen';
			}
		}
