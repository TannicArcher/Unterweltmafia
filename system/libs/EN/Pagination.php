<?php
	...
	class Pagination
	{
		...
		{
			// ... seperator
			return '<a href="#" onclick="var page = prompt(\'Introduceti un numer intre 1 si ' . $this->GetNumPages() . '.\'); if( page > 0 &amp;&amp; page &lt;= ' . $this->GetNumPages() . ' ){ NavigateTo(\''.$this->GetLinkHref("'+page+'").'\'); } return false;" class="seperator">...</a>';
		}
		...
		function GetPageLinks()
		{
			...
			{
				$links .= $begin / $this->per_page <= 1 ? '<span class="prev">Inapoi</span> ' : '<a href="'.$this->GetLinkHref($this->current_page-1).'" class="prev">Inapoi</a> ';
				...
				$links .= $this->current_page >= $this->GetNumPages() ? '<span class="next">Inainte</span>' : '<a href="'.$this->GetLinkHref($this->current_page+1).'" class="next">Inainte</a>';
			}
